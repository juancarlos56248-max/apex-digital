...
Deno.serve(async (req) => {
  try {
    ...
    const alerts = STOCKS.reduce((acc, stock) => {
      ...
      const threshold = ALERT_THRESHOLDS.find(t => changePct >= t && changePct < t + 5);
      if (threshold !== undefined) acc.push({ stock, estimatedPrice, changePct });
      return acc;
    }, []);

    if (alerts.length === 0) {
      return Response.json({ ok: true, emailsSent: 0, message: "No hay umbrales cruzados" });
    }
    ...
    const emailBody = `<div style="background:#050505;padding:32px;font-family:Inter,sans-serif;max-width:600px;margin:auto;">
      ...
      <div style="background:#0a0a0a;border:1px solid #1a1a1a;border-radius:12px;padding:24px;">
        ...
        <p style="color:#888;font-size:13px;margin-bottom:20px;">Las siguientes acciones han alcanzado nuevos máximos:</p>
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <thead><tr style="background:#111;">
            <th style="padding:8px 12px;text-align:left;color:#888;font-size:11px;text-transform:uppercase;">Símbolo</th>
            <th style="padding:8px 12px;text-align:left;color:#888;font-size:11px;text-transform:uppercase;">Empresa</th>
            <th style="padding:8px 12px;text-align:left;color:#888;font-size:11px;text-transform:uppercase;">Precio</th>
            <th style="padding:8px 12px;text-align:left;color:#888;font-size:11px;text-transform:uppercase;">Cambio</th>
          </tr></thead>
    ...
    return Response.json({ ok: true, emailsSent: validUsers.length, alerts: alerts.map(a => ({ symbol: a.stock.symbol, changePct: a.changePct.toFixed(1) })) });
  } catch (error) {
