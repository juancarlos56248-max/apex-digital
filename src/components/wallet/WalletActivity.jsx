  ...
  return <div>
    ...
    <AnimatePresence mode="wait">
      {activeTab === "inicio" ? <motion.div key="inicio" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 rounded-2xl border border-border bg-card p-5">
        ...
        {benefits.map(([Icon, label, desc]) => <div key={label} className="flex items-center gap-3 rounded-xl border border-border bg-card p-4"><div className="rounded-xl border border-chart-4/25 bg-chart-4/10 p-2.5"><Icon className="h-4 w-4 text-chart-4" /></div><div><p className="text-sm font-semibold">{label}</p><p className="text-xs text-muted-foreground">{desc}</p></div><CheckCircle className="ml-auto h-4 w-4 text-success" /></div>)}
      </motion.div>}
    </AnimatePresence>
  </div>;
}