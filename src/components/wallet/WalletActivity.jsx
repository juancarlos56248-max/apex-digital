  ...
  return <div>
    ...
    <AnimatePresence mode="wait">
      {activeTab === "inicio" ? <motion.div key="inicio" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 rounded-2xl border border-border bg-card p-5">
        ...
        <div className="flex flex-col items-center py-10 text-center"><div className="rounded-2xl border border-gold/15 bg-gold/5 p-4"><ShoppingBag className="h-6 w-6 text-gold/50" /></div><p className="mt-3 text-sm font-semibold">Sin movimientos aún</p><p className="mt-1 text-xs text-muted-foreground">Tus transacciones aparecerán aquí</p></div>
