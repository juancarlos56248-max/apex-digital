...
export default function LockedWallet({ user, totalInvested, minimum }) {
  ...
  return <div className="mx-auto max-w-md space-y-5">
    <div><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-chart-3">APEX // DIGITAL VAULT</p><div className="mt-2 flex items-center gap-2"><h1 className="text-2xl font-black">Billetera Digital</h1><span className="rounded-full border border-chart-4/30 bg-chart-4/10 px-2 py-0.5 text-[9px] font-bold text-chart-4">EXCLUSIVA</span></div><p className="mt-1 text-sm text-muted-foreground">Solicita tu tarjeta · <span className="text-chart-3">5% cashback</span> en cada compra</p></div>
    ...
    <div className="rounded-2xl border border-chart-3/20 bg-card p-5">
      ...
      <div className="mt-3 h-2 overflow-hidden rounded-full bg-secondary"><motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} className="h-full rounded-full bg-gradient-to-r from-chart-4 to-chart-3" /></div>
